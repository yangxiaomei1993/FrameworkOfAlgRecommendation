...
public class ClassifierTree 
  ...
  protected int s_Level;

  protected int s_TreeDepth;
...
//    System.out.println(currLastID);
    m_id = currLastID;
    if (m_sons != null) {
      for (int i = 0; i < m_sons.length; i++) {
	currLastID = m_sons[i].assignIDs(currLastID);//currLastID在一直增加,初始的根为0，然后递增的往下增加
      }
    }
    return currLastID;
  }

  //添加个Level信息就可以解决大部分的问题, 从0开始编号只是为了方便java语言下标处理
  public void assignLevels(int fatherLevel){
      int currLevel = fatherLevel + 1;
//      System.out.println(currLevel);
      s_Level = currLevel;
      if(m_sons != null){
          for(int i = 0; i < m_sons.length; i++){
              m_sons[i].assignLevels(currLevel);
          }
      }
  ...
  public int treeDepth(){
      ...
      if(m_isLeaf){
          ...
          for (int i=0;i<m_sons.length;i++){
                int sonDepth = m_sons[i].treeDepth();
                if(temp < sonDepth){
                    temp = sonDepth;
                }
          }
...
//
  ...
  public Distribution getPrunInstances(){
      return this.m_test;
  }
