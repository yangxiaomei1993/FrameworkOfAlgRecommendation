 ...
 * @author String
 */
public class newMetrics extends dataProcess {

	protected double[] s_freqOneItem;
	protected double[] s_freqTwoItems;
	...
	public newMetrics(String fileName) throws Exception {
		super(fileName);
		readDataSet();
	}
	...
	public void setFreqOfOneItem() {
		...
		for (int i = 0; i < vec.size(); i++) {
			s_freqOneItem[i] = Double.parseDouble(vec.elementAt(i).toString())
...
//		System.setOut(ps);
		...
		for (int i = 0; i < fileNames.length; i++) {
			...
			newMetrics newmetrics = new newMetrics(filePath + fileName + ".arff");
			if (newmetrics.s_isOnlyClass) {
				continue;
			}

			newmetrics.computeMetrics();
			String line = newmetrics.outputFeaVector();
			System.out.println((i + 1) + "," + fileName + "," + line);
		}

		// String fileName =
		// "C:\\Documents and Settings\\String\\妗岄潰\\weather.arff";
		// newMetrics newmetrics = new newMetrics(fileName);
		// newmetrics.computeMetrics();
		// String line = newmetrics.outputFeaVector();
